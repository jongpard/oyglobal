...
def compute_diffs_and_blocks(df_today: pd.DataFrame, df_prev: Optional[pd.DataFrame], prev_path: Optional[str]):
    ...
    up_lines = [f"- <{r['url']}|{r['name']}> {int(r['rank_prev'])}위 → {int(r['rank_today'])}위 (↑{int(r['delta'])})" for _, r in up10.iterrows()]
    ...
    ins = len(df_today[~df_today["url"].isin(df_prev["url"])])
