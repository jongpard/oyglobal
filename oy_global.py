...
DENY_RE  = re.compile(r'/member/|/myaccount/|/account|/brand($|/)|/display/page/|/event/|/flash-deal|/new-arrivals|/category/|/search', re.I)
ONCLICK_DETAIL_RE = re.compile(r"""['"](?P<url>/(?:product|goods)[^'"]+)['"]""")

def _ensure_debug_dirs(): os.makedirs(DEBUG_DIR, exist_ok=True)
...
def _text_from_attrs(tag) -> str:
    for a in ["aria-label","title","data-name","data-goods-nm","data-product-name"]:
        if tag and tag.has_attr(a) and tag[a].strip(): return tag[a].strip()
    return ""
...
def _dedupe(df: pd.DataFrame) -> pd.DataFrame:
    if "url" in df.columns: df = df.drop_duplicates(subset=["url"], keep="first")
...
def _recover_href_from_data(el) -> str:
    for a in ["data-url","data-href","data-link","data-detail-url","data-product-url"]:
        if el.has_attr(a) and el[a]: return _normalize_href(el[a])
    for a in ["data-product-id","data-goods-no","data-ref-goodsno","data-prd-no","data-prdno","data-goodsno"]:
        if el.has_attr(a) and el[a]: return _normalize_href(f"/product/detail?prdNo={el[a]}")
    return ""
...
def _extract_link(card) -> str:
    for a in card.select('a[href], a[onclick]'):
        ...
        oc = _recover_href_from_onclick(a)
        ...
        for k in ["data-url","data-href","data-link","data-detail-url","data-product-url"]:
            if a.has_attr(k) and a[k]:
                ...
                if _is_product_link(h): return h
    h2 = _recover_href_from_data(card)
...
async def _harvest_from_dom(html: str) -> pd.DataFrame:
    ...
    if not cards:
        ...
        for a in anchors:
            ...
            ok = _is_product_link(hn) or _is_product_link(_recover_href_from_onclick(a))
    ...
    for card in cards:
        ...
        rows.append({
            ...
            "discount_pct": disc,
            "price_str": f"US${sale:.2f}" if sale is not None else (f"US${original:.2f}" if original else ""),
            "url": link,
        })
        rank += 1
        if rank > 120: break
        ...
        for lst in _flatten_lists_from_json(data):
            for i, prod in enumerate(lst, start=1):
                ...
                rows.append({
                    ...
                    "sale_price": sale, "original_price": ori, "discount_pct": disc,
                    "price_str": f"US${sale:.2f}" if sale is not None else (f"US${ori:.2f}" if ori else ""),
                    "url": url
                })
